...
function PoolDetails({ wallet, busy, setBusy, setNotice }: PageProps) {
  ...
  const run = async (action: 'join' | 'contribute') => {
    ...
    try {
      ...
      const submission = action === 'join'
        ? await joinSahaPool(wallet, { contractAddress: address, eligibilitySecret: secret })
        : await contributeConfidentially(wallet, { contractAddress: address, eligibilitySecret: secret, amount: BigInt(amount) });
      setNotice({ kind: 'success', text: `The wallet accepted the transaction. Exact transaction hash: ${submission.transactionHash}` });
    } catch (error) { setNotice({ kind: 'error', text: humaniseMidnightError(error) }); }
    finally { setBusy(null); }
  };
